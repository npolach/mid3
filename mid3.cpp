...
int unitTest(double tol)
{
    ...
    for (int i=0; i < 480; i+=3) {
        ...
        height =    data[i+1];
        ponderal =  data[i+2];
        returned = getPonderal(mass, height);

        printf("%f\t%f\t%f\t%f", mass, height, ponderal, returned);
        if (((returned-ponderal)>tol) ||        // Higher than allowed tolerance
                ((returned-ponderal)<(-tol))) { // Less than allowed tolerance
            printf("  <-----outside of tolerance");
            errorCount++;
        }
        printf("\n");
    }
